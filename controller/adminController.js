...
module.exports = {
  ...
  postAdminLogin:(req,res)=>{
    ...
    if(user.username===ADMIN_USER && user.password===ADMIN_PASS)
    {
      ...
      res.send({status:false,url:'/admin/login',message:"you are not admin,admin details is wrrong"})
  ...
  updattingProduct:async(req,res)=>{
    ...
    try {
      ...
      const status = await Product.updateOne({_id:id},{$set:{name,description,price,imagePath}})
      console.log(status);
      // await newProduct.save();
      res.redirect("/admin/show");
    } catch (error) {
